...
<head>
<meta charset="utf-8">
<title>两点兔兔 - 致力于给予最好的东西给用户</title>
                        		...
                        		<div class="col-lg-6 col-sm-12 col-xs-12" >
								    ...
								    <a href="${pageContext.request.contextPath}/zyxq?rid=${r.rid}">
									    ...
									    <div class="caption">
                                          ...
                                          <!--       SSM分页 -->
			       <div class="pager" >
			          <font size="2">共<font size="4"><strong> ${page.totalPageCount}</strong></font> 页</font> <font size="2">当前第  
			            <font size="4"><strong>${page.pageNow}</strong></font> 页</font> <a href="index?pageNow=1" class="btn btn-primary ">首页 </a>  
			        <c:choose>  
			            <c:when test="${page.pageNow - 1 > 0}">  
			                <a href="index?pageNow=${page.pageNow - 1}" class="btn btn-info">上一页</a>  
			            </c:when>  
			            <c:when test="${page.pageNow - 1 <= 0}">  
			                <a href="index?pageNow=1" class="btn btn-info">上一页</a>  
			            </c:when>  
			        </c:choose>  
			        <c:choose>  
			            <c:when test="${page.totalPageCount==0}">
			                <a href="index?pageNow=${page.pageNow}" class="btn btn-info">下一页</a>  
			            </c:when>  
			            <c:when test="${page.pageNow + 1 < page.totalPageCount}">  
			                <a href="index?pageNow=${page.pageNow + 1}" class="btn btn-info">下一页</a>  
			            </c:when>  
			            <c:when test="${page.pageNow + 1 >= page.totalPageCount}">  
			                <a href="index?pageNow=${page.totalPageCount}" class="btn btn-info">下一页</a>  
			            </c:when>  
			        </c:choose>  
			        <c:choose>  
			            <c:when test="${page.totalPageCount==0}">  
			                <a href="index?pageNow=${page.pageNow}" class="btn btn-success">尾页</a>  
			            </c:when>  
			            <c:otherwise>  
			                <a href="index?pageNow=${page.totalPageCount}" class="btn btn-success">尾页</a>  
			            </c:otherwise>  
			        </c:choose> 
                        </div>
