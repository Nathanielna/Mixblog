	...
	<nav class="navbar navbar-default" role="navigation">
        ...
        <div class="collapse navbar-collapse navbar-ex1-collapse">
            <ul class="nav navbar-nav ">
                ...
                <li class="active"><a href="${pageContext.request.contextPath}/zxsp">在线视频</a></li>
            </ul>
        ...
        <div class="row">
            ...
            <div class="col-lg-10">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-lg-9">
                          <h3>在线视频需要在地址栏按下回车就可以正常观看(实在无法播放就下载吧)</h3>
                         <div class="row">
                        	<c:forEach items="${rzxsplist}" var="r">
								    ...
								    <a href="${pageContext.request.contextPath}/zyxq?rid=${r.rid}">
									    ...
									    <div class="caption">
									        ...
									        <p>
                                       		...
                                       		<c:if test="${r.rca==4}">
                                       			<span class="label label-default">在线视频</span>
                                       		</c:if>
										    </p>
			       ...
			       <div class="pager" >
			          <font size="2">共 ${page.totalPageCount} 页</font> <font size="2">当前第  
			            ${page.pageNow} 页</font> <a href="zxsp?pageNow=1" class="btn btn-primary ">首页 </a>  
			        <c:choose>  
			            <c:when test="${page.pageNow - 1 > 0}">  
			                <a href="zxsp?pageNow=${page.pageNow - 1}" class="btn btn-info">上一页</a>  
			            </c:when>  
			            <c:when test="${page.pageNow - 1 <= 0}">  
			                <a href="zxsp?pageNow=1" class="btn btn-info">上一页</a>  
			            </c:when>  
			        </c:choose>  
			        <c:choose>  
			            <c:when test="${page.totalPageCount==0}">
			                <a href="zxsp?pageNow=${page.pageNow}" class="btn btn-info">下一页</a>  
			            </c:when>  
			            <c:when test="${page.pageNow + 1 < page.totalPageCount}">  
			                <a href="zxsp?pageNow=${page.pageNow + 1}" class="btn btn-info">下一页</a>  
			            </c:when>  
			            <c:when test="${page.pageNow + 1 >= page.totalPageCount}">  
			                <a href="zxsp?pageNow=${page.totalPageCount}" class="btn btn-info">下一页</a>  
			            </c:when>  
			        </c:choose>  
			        <c:choose>  
			            <c:when test="${page.totalPageCount==0}">  
			                <a href="zxsp?pageNow=${page.pageNow}" class="btn btn-success">尾页</a>  
			            </c:when>  
			            <c:otherwise>  
			                <a href="zxsp?pageNow=${page.totalPageCount}" class="btn btn-success">尾页</a>  
			            </c:otherwise>  
			        </c:choose> 
                        </div>
