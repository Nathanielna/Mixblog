				  ...
				  <li class="active">会员列表</li>
                    ...
                    <table class="table table-hover">
                            ...
                            <c:forEach items="${ulist}" var="u">
                                ...
                                <td>${u.ubalance}</td>
                                <td>
                                    <a class="btn btn-primary" href="${pageContext.request.contextPath}/manageruser?uid=${u.uid}" role="button">编辑</a>
                                    <a class="btn btn-danger" href="${pageContext.request.contextPath}/aduserdel?uid=${u.uid}" role="button">删除</a>
                                </td>
                              </tr>
                            </c:forEach>
                            </tbody>
			       ...
			       <div class="pager" >
			          <font size="2">共 ${page.totalPageCount} 页</font> <font size="2">当前第  
			            ${page.pageNow} 页</font> <a href="adminuser?pageNow=1" class="btn btn-primary ">首页 </a>  
			        <c:choose>  
			            <c:when test="${page.pageNow - 1 > 0}">  
			                <a href="adminuser?pageNow=${page.pageNow - 1}" class="btn btn-info">上一页</a>  
			            </c:when>  
			            <c:when test="${page.pageNow - 1 <= 0}">  
			                <a href="adminuser?pageNow=1" class="btn btn-info">上一页</a>  
			            </c:when>  
			        </c:choose>  
			        <c:choose>  
			            <c:when test="${page.totalPageCount==0}">
			                <a href="adminuser?pageNow=${page.pageNow}" class="btn btn-info">下一页</a>  
			            </c:when>  
			            <c:when test="${page.pageNow + 1 < page.totalPageCount}">  
			                <a href="adminuser?pageNow=${page.pageNow + 1}" class="btn btn-info">下一页</a>  
			            </c:when>  
			            <c:when test="${page.pageNow + 1 >= page.totalPageCount}">  
			                <a href="adminuser?pageNow=${page.totalPageCount}" class="btn btn-info">下一页</a>  
			            </c:when>  
			        </c:choose>  
			        <c:choose>  
			            <c:when test="${page.totalPageCount==0}">  
			                <a href="adminuser?pageNow=${page.pageNow}" class="btn btn-success">尾页</a>  
			            </c:when>  
			            <c:otherwise>  
			                <a href="adminuser?pageNow=${page.totalPageCount}" class="btn btn-success">尾页</a>  
			            </c:otherwise>  
			        </c:choose> 
			       </div>
            </div>
